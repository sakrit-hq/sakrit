...
``spec.v1`` parses/validates/emits a SED v1 document; the builtin normalizers live in
``sakrit.core.normalizers`` (a fingerprint primitive) and are re-exported here for the
public format API (conformance-gated by ``tests/fixtures/`` golden vectors);
``spec.convergence`` binds a document to the in-code ``EffectDecl``.
"""

from sakrit.core.normalizers import BUILTIN_NORMALIZERS, NORMALIZER_NAMES, normalize
from sakrit.spec.convergence import decl_to_sed, load_sed, sed_to_decl
